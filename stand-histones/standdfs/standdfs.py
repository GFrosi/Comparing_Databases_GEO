...
def merge_df_left(list_df):
    '''Receives a list of df
    (df_maps). Returns a merged
    df including all columns from
    metadata databases.'''

    df_merged = reduce(lambda left,right: pd.merge(left,right, on=['GSM'], how='left').fillna('----'), list_df) #union
...
def filter_histones_inputs(df, col_target, col_gse):
        ...
        df_hist = df[df[col_target].str.contains('|'.join(list_hist), case=False, na=False)]
