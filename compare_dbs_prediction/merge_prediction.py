...
def merge_dfs(df_hist, df_pred):
    ...
    df_result = df_hist.merge(df_pred, how='left', left_on='Experimental-id', right_on='md5sum').fillna('----') #updating col names
...
def main():
    ...
    df_pred = pd.read_csv(sys.argv[2],sep='\t') # df with Predicted and Max_value prediction from EpiLaP
    # df_result = replace_noCAsample(df_hist, df_pred)
    df_result = merge_dfs(df_hist, df_pred)
    to_date = date.today()
    df_result.to_csv('Histones_DBs_filled_CA_pred_' + str(to_date) + '.tsv' , sep = '\t',index=False)
    print('File saved!')
